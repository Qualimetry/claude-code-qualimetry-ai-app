...
- **Surfaces review findings the moment you open a reviewed file.** Read any source file in your repo and Claude immediately shows you which standards, principles, and policy violations have been raised against it, plus a standards-compliant example of how to resolve them. No command, no manual lookup. When the current branch has an open pull request, the findings are automatically scoped to the issues raised on the pull request's new code.
...
- **Triages rules-based analysis findings.** Existing bugs, security vulnerabilities, and code smells from your static analysis (BLOCKER / CRITICAL / MAJOR / MINOR / INFO) are accessible from chat. Claude walks them in priority order: security first, reliability second, quality last. It also assesses the security hotspots your analysis raises - the security-sensitive code a person still has to sign off - and tells you which ones look safe and which need work. Supply a pull-request number to target the issues raised on the pull request's new code instead of the whole branch. *(Qualimetry Enterprise.)*
- **Resolves dependency CVEs by upgrading to the next safe version.** Claude pulls the CVE list for your current branch, locates the manifest files, and proposes upgrades to the `NextSafeVersion` for each vulnerable package - auto-applying low-risk upgrades and asking before medium/high-risk ones, then re-validating with a build. It reads your suppressions too, so it skips risk you have already agreed to accept, leaves alone anything waiting on an approval, and warns you when a suppression is about to expire and bring the finding back. *(Qualimetry Enterprise.)*
...
> **No extra installs needed.** The automatic on-open findings use only `git` and `curl` (or `wget`), which ship with every environment Claude Code runs in. If `jq` happens to be installed, the findings appear as a grouped per-pillar digest; without it you get the raw findings payload — same information, less formatting.
...
**Issues do not appear when you open a file** — the file may not have been reviewed yet on the Qualimetry server, or the branch has no review. The hook resolves the repository name from your `origin` remote (`owner/repo`) and falls back to the folder name automatically. Run `/compliance-check <file>` manually to confirm whether the server has findings for that path.
