...
Also covers security hotspots: security-sensitive code that needs a person to decide whether it is safe. Hotspots are not issues and never appear in the issue results, so a branch with every issue cleared can still have hotspots outstanding. The agent reads the rule's risk description and fix guidance, assesses the code, and reports back. Recording the review decision stays with the user.
...
Suppression-aware throughout: a CVE with a suppression request awaiting approval is reported rather than fixed, an approved suppression about to expire is flagged before the finding returns, and the CVEs already removed from the report can be listed with the scope, requester and reason behind each one. Raising and approving suppressions stays in the Qualimetry dashboard.
...
Generates Blitzy rules packs from the organisation's standards so Blitzy produces compliant code. Two packs:

- **Coding standards pack** (Qualimetry AI & Enterprise) - policies, principles, and (when language codes are supplied) language coding standards via `get_coding_standards_blitzy`.
- **Analysis rules pack** (Qualimetry Enterprise) - the rules-based analysis rule set for a language via `get_rules_based_analysis_rules_blitzy`.

Each pack is a single JSON object with `rules` (machine-readable, for API push) and `files` (path/content pairs mirroring the zip tree a client can write to disk or a repository).

**Usage:** Invoke with `/blitzy-rules` to generate a pack.

## Product Availability

The `coding-standards` and `review-check` skills work with both **Qualimetry AI** and **Qualimetry Enterprise**. The `analysis-issues` and `dependency-check` skills and their MCP tools require **Qualimetry Enterprise**. The `blitzy-rules` skill works with both deployments for the coding-standards pack; its analysis rules pack (`get_rules_based_analysis_rules_blitzy`) requires **Qualimetry Enterprise**.
...
cp -r /tmp/qualimetry-ai-skills/blitzy-rules ~/.agents/skills/
```
...
These skills use the following Qualimetry MCP tools. Qualimetry Enterprise serves all 21; Qualimetry AI serves 12.

### Standards, principles and review findings

Served by the Qualimetry Review Service on both products.
...
| `get_all_review_issues` | review-check | Fetch all review issues for a file (optional `pullRequest` scopes to a PR's new code) | Qualimetry AI & Enterprise |
| `get_coding_standards_review_issues` | review-check (fallback) | Fetch coding standards issues (optional `pullRequest` PR scope) | Qualimetry AI & Enterprise |
| `get_design_best_practice_review_issues` | review-check (fallback) | Fetch design best practice issues (optional `pullRequest` PR scope) | Qualimetry AI & Enterprise |
| `get_general_principles_review_issues` | review-check (fallback) | Fetch general principles issues (optional `pullRequest` PR scope) | Qualimetry AI & Enterprise |
| `get_secure_principles_review_issues` | review-check (fallback) | Fetch secure principles issues (optional `pullRequest` PR scope) | Qualimetry AI & Enterprise |
| `get_policies_review_issues` | review-check (fallback) | Fetch policy violation issues (optional `pullRequest` PR scope) | Qualimetry AI & Enterprise |
...
| `get_coding_standards_blitzy` | blitzy-rules | Generate a Blitzy rules pack from policies, principles, and (when language codes are supplied) language coding standards | Qualimetry AI & Enterprise |

### Rules-based analysis

Served from the analysis server behind a Qualimetry Enterprise deployment. Every tool in this group is named `get_rules_based_analysis_*`.

| Tool | Used By | Purpose | Availability |
|------|---------|---------|--------------|
| `get_rules_based_analysis_issues_summary` | analysis-issues | Fetch issue counts by type and severity, plus the security hotspot counts, for a repo/branch (optional `pullRequest` scopes to a PR) | Qualimetry Enterprise |
| `get_rules_based_analysis_issues` | analysis-issues | Fetch paginated analysis issues with type/severity/file filtering, each marked `Analysis` or `Dependencies` (optional `pullRequest` for PR new code) | Qualimetry Enterprise |
| `get_rules_based_analysis_security_hotspots` | analysis-issues | Fetch the paginated security hotspots for a repo/branch, filtered by review status, priority, security category or file. Hotspots are never returned by the issue tools. | Qualimetry Enterprise |
| `get_rules_based_analysis_security_hotspot` | analysis-issues | Fetch one security hotspot with the rule's risk description, exploit description and fix recommendations | Qualimetry Enterprise |
| `get_rules_based_analysis_rules` | analysis-issues | Fetch the analysis-engine rule definitions for a language, optionally scoped to a quality profile | Qualimetry Enterprise |
| `get_rules_based_analysis_rules_blitzy` | blitzy-rules | Generate a Blitzy rules pack from the analysis-engine rule set for a language | Qualimetry Enterprise |

### Dependencies

Served from the dependency scan and the upgrade advisor behind a Qualimetry Enterprise deployment. No analysis server is involved.

| Tool | Used By | Purpose | Availability |
|------|---------|---------|--------------|
| `get_dependency_vulnerabilities` | dependency-check | Fetch all dependency CVE vulnerabilities with inline upgrade advice (next safe version, latest version, upgrade risk) and suppression flags for a repo/branch. Optionally filter to one CVE or include the CVEs a suppression removed. | Qualimetry Enterprise |
| `get_dependency_suppressions` | dependency-check | Fetch the dependency suppressions that apply to a branch: what has been accepted, by whom, why, how widely, and when it expires | Qualimetry Enterprise |
...
Marking a security hotspot as reviewed, and raising or approving a suppression, are human decisions recorded in Qualimetry. Every MCP tool is read-only.
